...
- `waFindDeviceByID(const char *guid, void **out)` — busca por GUID estável
- `waFindDeviceByName(const char *name, void **out)` — fallback por nome
...
curl -s http://localhost:8080/v1/devices | jq '.devices[].host_api'
```
...
curl -s http://localhost:8080/v1/devices | jq '.devices[].id'
